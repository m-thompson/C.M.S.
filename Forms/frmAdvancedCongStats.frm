...
Begin VB.Form frmAdvancedCongStats 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "C.M.S. Advanced Congregation Stats"
   ClientHeight    =   9390
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8595
   Icon            =   "frmAdvancedCongStats.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton cmdExport 
      ...
      Width           =   930
   End
   Begin VB.CommandButton cmdOpenAnother 
      Caption         =   "Screen 2..."
      Height          =   345
      Left            =   1920
      TabIndex        =   18
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   225
      Width           =   8230
      Begin VB.ComboBox cmbBookgroups2 
         Height          =   315
         Left            =   3630
      ...
      Begin VB.ComboBox cmbNames2 
         ...
         Width           =   2685
      End
      Begin VB.ComboBox cmbWhichPeople2 
         Height          =   315
         Left            =   1440
         Style           =   2  'Dropdown List
         TabIndex        =   5
   ...
   End
End
Attribute VB_Name = "frmAdvancedCongStats"
...
Dim mlBookGroup2 As Long, mlThePerson2 As Long, mbReportsUpdated As Boolean, mbChangedCurrentPubsFlag As Boolean
Dim mlNoRows As Long, mbForm1RebuiltTable As Boolean
...
End Sub
Private Sub cmbBookgroups2_Click()
On Error GoTo ErrorTrap

    If cmbBookgroups2.ListIndex > -1 Then
        mlBookGroup2 = cmbBookgroups2.ItemData(cmbBookgroups2.ListIndex)
    Else
        mlBookGroup2 = 0
    End If
...
End Sub
Private Sub cmbNames2_Click()
On Error GoTo ErrorTrap

    If cmbNames2.ListIndex > -1 Then
        mlThePerson2 = cmbNames2.ItemData(cmbNames2.ListIndex)
    Else
        mlThePerson2 = 0
    End If
...
On Error GoTo ErrorTrap
    ...
    Case 0 'all persons
        cmbWhichPeople2.ListIndex = 0
        cmbWhichPeople2.Enabled = False
...
End Sub
Private Sub cmbWhichPeople2_Click()
On Error GoTo ErrorTrap

    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 9 'person
        cmbNames2.Visible = True
        cmbBookgroups2.Visible = False
        cmbAttribute.ListIndex = 0 'for a specific person don't need attribute set
        cmbAttribute.Enabled = False
        cmbAttribute2.ListIndex = 0 'for a specific person don't need attribute set
        cmbAttribute2.Enabled = False
    Case 12 'Group
        cmbNames2.Visible = False
        cmbBookgroups2.Visible = True
        cmbAttribute.Enabled = True
        cmbAttribute2.Enabled = True
    Case Else
        cmbNames2.Visible = False
...
Private Sub cmdOpenAnother_Click()
On Error Resume Next
    frmAdvancedCongStats2.Show vbModeless, frmMainMenu
    If Err.number <> 0 Then
        frmAdvancedCongStats2.Show vbModal, frmMainMenu
    End If
End Sub
...
Dim bArtificialRegPio As Boolean, bArtificialSpecPio As Boolean, lHourCredit As Long
    ...
    mbForm1RebuiltTable = True
    frmAdvancedCongStats2.Form2RebuiltTable = False
...
'    End Select
    ...
    If mbDatesChanged Or mbCreditOptionChanged Or mbReportsUpdated _
        Or frmAdvancedCongStats2.Form2RebuiltTable Or mbChangedCurrentPubsFlag Then
    ...
    'elder / servant criteria
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 5 'elders. Only include men appointed prior to startdate
        sElderStr = sElderStr & " AND PersonID IN " & _
                    "(SELECT PersonID FROM tblEldersAndServants " & _
                    " WHERE AppointmentDate <= # " & EndDate_US & "# " & _
                    " AND ElderOrServant = 'E') "
    Case 6 'servants
    ...
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 9 'person
        strPerson = strPerson & " AND PersonID = " & mlThePerson2 & " "
    End Select
    ...
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 12 'bookgroups
        sBookGroup = sBookGroup & " AND BookGroupID = " & mlBookGroup2 & " "
    End Select
    ...
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 2
        MinTypeSQL = MinTypeSQL & " AND MinType = " & IsPublisher & " "
    Case 3
        MinTypeSQL = MinTypeSQL & " AND MinType = " & IsRegPio & " "
    Case 4
        MinTypeSQL = MinTypeSQL & " AND MinType = " & IsAuxPio & " "
    End Select
    ...
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 10
        GenderSQL = GenderSQL & " AND GenderMF = 'M' "
    Case 11
        GenderSQL = GenderSQL & " AND GenderMF = 'F' "
    End Select
    ...
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 7
        BaptismSQL = BaptismSQL & " AND IsBaptised = True "
    Case 8
        BaptismSQL = BaptismSQL & " AND IsBaptised = False "
    End Select
    ...
    Select Case cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    Case 9 'person
        msCriteria = msCriteria & " and " & _
            CongregationMember.FullNameFromDB(mlThePerson2) & " " & vbCrLf
    Case 12 'bookgroup
        msCriteria = msCriteria & " and " & _
            GetGroupName(mlBookGroup2) & " " & vbCrLf
    Case Is > 0
        msCriteria = msCriteria & " and " & LCase$(cmbWhichPeople2.text) & " " & vbCrLf
    End Select
...
'    cmbYear.text = CStr(ServiceYear(CDate("01/" & TheString)))
    ...
    '
    'Now populate cmbWhichPeople2. Used to select category of person for which to
    ' calc totals/averages
    '
    cmbWhichPeople2.AddItem "[NONE]"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 0
    cmbWhichPeople2.AddItem "All Persons"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 1
    cmbWhichPeople2.AddItem "Publishers only"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 2
    cmbWhichPeople2.AddItem "Regular Pioneers"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 3
    cmbWhichPeople2.AddItem "Auxiliary Pioneers"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 4
    cmbWhichPeople2.AddItem "Elders"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 5
    cmbWhichPeople2.AddItem "Ministerial Servants"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 6
    cmbWhichPeople2.AddItem "Baptised Publishers"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 7
    cmbWhichPeople2.AddItem "Unbaptised Publishers"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 8
    cmbWhichPeople2.AddItem "Males"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 10
    cmbWhichPeople2.AddItem "Females"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 11
    
    cmbWhichPeople2.AddItem "This person >>>"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 9
    
    cmbWhichPeople2.AddItem "This group >>>"
    cmbWhichPeople2.ItemData(cmbWhichPeople2.NewIndex) = 12
    ...
    PopulateComboWithCurrentPublishers Me!cmbNames, _
                                       IsPublisher
    PopulateComboWithCurrentPublishers Me!cmbNames2, _
                                       IsPublisher
    ...
    HandleListBox.PopulateListBox Me.cmbBookgroups2, _
                              "SELECT GroupNo, GroupName " & _
                              "FROM tblBookGroups " & _
                              "ORDER BY 2 ASC", CMSDB, 0, "", False, 1
...
On Error GoTo ErrorTrap
Dim TheControl As Control, lPeople1 As Long, lPeople2 As Long
    ...
    For Each TheControl In Me.Controls
        If TypeOf TheControl Is ComboBox Then
            If TheControl.ListIndex = -1 And _
                ...
                TheControl.Name <> "cmbNames" And _
                TheControl.Name <> "cmbNames2" Then
    ...
    If cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex) = 7 And _
        (cmbAttribute.ItemData(cmbAttribute.ListIndex) = 2 Or _
         cmbAttribute2.ItemData(cmbAttribute2.ListIndex) = 2) Then
        MsgBox "Cannot process 'Years Baptised' for 'Unbaptised Publishers'!.", _
                vbOKOnly + vbExclamation, AppName
    ...
    'check for valid combinations of WhichPersons...
    lPeople1 = cmbWhichPeople.ItemData(cmbWhichPeople.ListIndex)
    lPeople2 = cmbWhichPeople2.ItemData(cmbWhichPeople2.ListIndex)
    
    Select Case lPeople1
    Case 0
        Select Case lPeople2
        Case Is > 0
            ValidQuery = False
        End Select
    Case 1, 2, 3
        Select Case lPeople2
        Case 0, 5, 6, 7, 8, 9, 10, 11, 12
        Case Else
            ValidQuery = False
        End Select
    Case 4, 5
    ...
    Case 9, 10
        Select Case lPeople2
        Case 0, 1, 2, 3, 4, 7, 8, 12
        Case Else
            ValidQuery = False
        End Select
    Case 11
        Select Case lPeople2
        Case 0, 2, 3, 4, 5, 6, 7, 10, 11
        Case Else
            ValidQuery = False
        End Select
    Case Else
    ...
    If Not ValidQuery Then
        MsgBox "Invalid query", vbOKOnly + vbExclamation, AppName
        cmbWhichPeople2.SetFocus
    End If
...
ErrorTrap:
    EndProgram
End Sub
Private Sub cmbNames2_KeyDown(KeyCode As Integer, Shift As Integer)
On Error GoTo ErrorTrap

    If KeyCode = 46 Then
        cmbNames2.ListIndex = -1
    End If
...
End Property
Public Property Let Form1RebuiltTable(ByVal vNewValue As Boolean)
    mbForm1RebuiltTable = vNewValue
End Property

Public Property Get Form1RebuiltTable() As Boolean
    Form1RebuiltTable = mbForm1RebuiltTable
End Property
