...
Begin VB.Form frmAdvancedReporting 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "C.M.S. Advanced Reporting"
   ClientHeight    =   9075
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   15180
   Icon            =   "frmAdvancedReporting.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   60
      Width           =   14985
      Begin VB.CommandButton cmdInsertToForm 
         Caption         =   "Insert>>"
         Height          =   450
         Left            =   13965
         TabIndex        =   7
         ToolTipText     =   "Insert query"
         Top             =   4410
         Width           =   885
      End
   ...
   End
End
Attribute VB_Name = "frmAdvancedReporting"
...
Dim mbAllowInsertToOtherForm As Boolean
Dim WithEvents frmOpenQry As frmOpenUserQuery
Attribute frmOpenQry.VB_VarHelpID = -1
Event PassQuery(TheQueryString As String)

Private Sub cmdInsertToForm_Click()

On Error GoTo ErrorTrap

    RaiseEvent PassQuery(txtQuery)
...
On Error Resume Next
    ...
    frmAdvancedReporting2.Show vbModeless, frmMainMenu
    
    If Err.number <> 0 Then
        frmAdvancedReporting2.Show vbModal, frmMainMenu
    End If
...
On Error GoTo ErrorTrap
    ...
    cmdInsertToForm.Enabled = mbAllowInsertToOtherForm
...
Dim lMaxRows As Long, lRows As Long, sQuery As String
...
On Error GoTo ErrorTrap
    ...
    If Err.number <> 0 Then
        MsgBox "The following error occurred - check, amend and re-run the SQL." & _
                    vbCrLf & vbCrLf & Err.Description, vbExclamation + vbOKOnly, AppName
...
'            lstTables.ListIndex = GetListBoxRowFromTwips(lstTables, Y)
        Else
            Exit Sub
        End If
...
'            lstTables.Selected(CInt(TheRow)) = True
...
End Property
Public Property Let AllowInsertToOtherForm(ByVal vNewValue As Boolean)
    mbAllowInsertToOtherForm = vNewValue
End Property
