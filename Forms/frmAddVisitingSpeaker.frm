VERSION 5.00
Begin VB.Form frmAddVisitingSpeaker 
   BorderStyle     =   1  'Fixed Single
   Caption         =   " C.M.S. Add/Edit Visiting Speaker"
   ClientHeight    =   3705
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5265
   Icon            =   "frmAddVisitingSpeaker.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "frmAddVisitingSpeaker"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim mlPersonID As Long, mlCongNo As Long, mbAddMode As Boolean
...
On Error GoTo ErrorTrap
    ...
    If mlCongNo = 0 Then
        MsgBox "No congregation selected", vbOKOnly + vbExclamation, AppName
        Unload Me
    End If

    If chkCoord.value = vbUnchecked And chkSpeaker.value = vbUnchecked Then
        ShowMessage "Please select 'Speaker' or 'Coordinator'", 1000, Me
        Exit Sub
    End If
    ...
    If PersonalFieldsValidatedOK Then
        ...
        If mlPersonID = 0 Then
            'new rec - add to cong
            If chkSpeaker.value = vbChecked Then
                CongregationMember.MakePersonVisitingSpeaker mlCongNo, rs!MaxID
            End If
            If chkCoord.value = vbChecked Then
                CongregationMember.AddPersonToRole mlCongNo, 6, 14, 73, rs!MaxID
            End If
            mlPersonID = rs!MaxID
        Else
            If chkSpeaker.value = vbChecked Then
                CongregationMember.MakePersonVisitingSpeaker mlCongNo, mlPersonID
            Else
                CongregationMember.DeleteVisitingSpeaker mlPersonID
            End If
            If chkCoord.value = vbChecked Then
                CongregationMember.AddPersonToRole mlCongNo, 6, 14, 73, mlPersonID
            Else
                CongregationMember.DeletePersonFromRole mlCongNo, 6, 14, 73, mlPersonID
            End If
        End If
...
On Error GoTo ErrorTrap
    ...
    lblCongName = GetCongregationName(mlCongNo) & _
                    IIf(mlCongNo = 32767, "", " Congregation")
    ...
    If mlPersonID > 0 Then
        ...
        If CongregationMember.IsVisitingSpeaker(mlPersonID) Then
            chkSpeaker.value = vbChecked
        Else
            chkSpeaker.value = vbUnchecked
        End If
        If CongregationMember.DoesRole2(CInt(mlPersonID), 73) Then
            chkCoord.value = vbChecked
        Else
            chkCoord.value = vbUnchecked
        End If
        End With
    Else
        txtFirstName = ""
...
Public Property Get CongNo() As Long
    CongNo = mlCongNo
End Property

Public Property Let CongNo(ByVal vNewValue As Long)
    mlCongNo = vNewValue
End Property
